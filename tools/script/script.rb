...
def help()
    ...
    puts ""
    puts "	4.生成Command和对应的Event代码"
    puts "		命令：ruby script\\script.rb projectName ModuleName cmd cmdName"
    puts "		参数：projectName - 项目名称，首字母小写"
    puts "		      ModuleName - 模块名称，首字母大写"
    puts "		         cmdName - Command 名称，省略Command后缀：例如要创建SaveUserInfoCommand，直接写SaveUserInfo就可以"
end
...
#moduleName 模块名称
#par 参数
def create(action, moduleName, par)
  ...
  result = case action
    when 'create': create_module(moduleName, par)
    when 'view': create_view(moduleName, par)
    when 'res': create_respose(moduleName, par)
    when 'cmd': create_command(moduleName, par)
    when 'service': create_service(moduleName, par)
    else puts "ERROR: action not supported. Supported actions are command"
  end
end

#生成模块文件
def create_module(module_name, par)
  #配置初始化参数
  @module = Module.new
  @module.par = par
...
def create_view(module_name, view_name)
  #配置初始化参数
  @module = Module.new
...
def create_respose(module_name, response)
  #配置初始化参数
  @module = Module.new
  @module.par = response
  @module.module_name = module_name
...
def create_command(module_name, cmd)
  #配置初始化参数
  @module = Module.new
  @module.par = cmd
  @module.module_name = module_name
...
def create_service(service_name, cmd)
  #配置初始化参数
  @module = Module.new
  @module.par = cmd
...
def read_file(file_name) 
  data = ""
  f = File.open(file_name).each { |line| data += line }
  return data
end
...
if $0 == __FILE__
  ...
  result = case project 
    when 'help': help()
    else create(action, moduleName, par)
  end
end